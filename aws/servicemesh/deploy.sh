...
#        aws --endpoint-url $APPMESH_FRONTEND $MESHCMD delete-route --mesh-name votemesh --route-name _node 2>/dev/null
#    done
#    aws --endpoint-url $APPMESH_FRONTEND $MESHCMD delete-virtual-node --mesh-name votemesh --virtual-node-name queue_node 2>/dev/null
#}

create_mesh() {
    print "Creating service mesh"
    print "====================="
    cmd=( aws --endpoint-url $APPMESH_FRONTEND $MESHCMD create-mesh --cli-input-json file:///$DIR/config/mesh/mesh.json --query mesh.metadata.arn --output text )
    print "${cmd[@]}"
...
create_virtual_node() {
    service=$1
    cmd=( aws --endpoint-url $APPMESH_FRONTEND $MESHCMD create-virtual-node --cli-input-json file:///$DIR/config/virtualnodes/$service --query virtualNode.metadata.uid --output text )
    print "${cmd[@]}"
...
create_virtual_router() {
    service=$1
    cmd=( aws --endpoint-url $APPMESH_FRONTEND $MESHCMD create-virtual-router --cli-input-json file:///$DIR/config/virtualrouters/$service --query virtualRouter.metadata.uid --output text )
    print "${cmd[@]}"
...
create_virtual_route() {
    service=$1
    cmd=( aws --endpoint-url $APPMESH_FRONTEND $MESHCMD create-route --cli-input-json file:///$DIR/config/routes/$service --query route.metadata.uid --output text )
    print "${cmd[@]}"
