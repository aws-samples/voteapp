...
deploy_node() {
    node=$1
    uid=$(aws --endpoint-url $LATTICE_FRONTEND $MESHCMD describe-virtual-node --mesh-name votemesh --virtual-node-name ${node}-vn --query virtualNode.metadata.uid --output text)
